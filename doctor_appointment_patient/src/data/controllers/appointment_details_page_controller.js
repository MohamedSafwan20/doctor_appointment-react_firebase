...
export default class AppointmentDetailsPageController {
  ...
  static async downloadPrescription({ appointment }) {
    try {
      const currentUser = CookieService.getCookie("user");
      ...
      const appointmentPatientSnapshot = await getDoc(appointmentPatientRef);

      NetworkService.downloadFileFromUrl({
        url: appointmentPatientSnapshot.get("prescriptionUrl"),
      });
